...
class TestEntity:
    ...
    def test_label(self):
        ...
        e = cesiumpy.Label(
            position=(-110, 40, 0), text="label_text", fillColor="blue", scale=0.1
        )
    ...
    def test_billboard_icon(self):
        ...
        b = cesiumpy.Billboard(
            position=[135, 35, 0], image="xxx.png", scale=0.1, eyeOffset=(0, 10e4, 0)
        )
        ...
        b = cesiumpy.Billboard(
            position=[135, 35, 0], image="xxx.png", scale=0.1, pixelOffset=(0, 150)
        )
    ...
    def test_ellipse(self):
        e = cesiumpy.Ellipse(
            position=[-110, 40, 0], semiMinorAxis=25.0, semiMajorAxis=40.0
        )
    ...
    def test_cylinder(self):
        ...
        e = cesiumpy.Cylinder(
            position=(-70, 40, 0), length=10, topRadius=100, bottomRadius=200
        )
    ...
    def test_polyline(self):
        e = cesiumpy.Polyline(
            positions=[-77, 35, -77.1, 35], width=5, material=cesiumpy.color.RED
        )
    ...
    def test_corridor_cornertypes(self):
        e = cesiumpy.Corridor(
            positions=[-130, 40, -120, 30, -110, 40],
            width=100000,
        ...
        msg = (
            "The 'cornerType' trait of a Corridor instance must be a CornerType or None"
        )
        with pytest.raises_regexp(traitlets.TraitError, msg):
            cesiumpy.Corridor(
                positions=[-130, 40, -120, 30, -110, 40],
    ...
    def test_rectangle(self):
        ...
        e = cesiumpy.Rectangle(
            ...
            material=cesiumpy.color.GREEN,
            outlineColor=cesiumpy.color.RED,
        )
        ...
        e = cesiumpy.Rectangle(
            coordinates=[(-80, 20), (-60, 40)], material="green", outlineColor="red"
        )
        self.assertEqual(e.script, exp)
        ...
        with pytest.raises_regexp(traitlets.TraitError, msg):
            cesiumpy.Rectangle(
                coordinates=[1, 2, 3], material="green", outlineColor="red"
            )

        msg = "The 'north' trait of a Rectangle instance must be a float"
        with pytest.raises_regexp(traitlets.TraitError, msg):
            cesiumpy.Rectangle(
                coordinates=(-80, 20, -60, "X"), material="green", outlineColor="red"
            )
    ...
    def test_polygon(self):
        ...
        exp = (
            "{polygon : {hierarchy : Cesium.Cartesian3.fromDegreesArray([1, 1, 2, 2])}}"
        )
        self.assertEqual(e.script, exp)
    ...
    def test_entities_repr(self):
        ...
        e = cesiumpy.Ellipse(
            position=[-110, 40, 0], semiMinorAxis=25e4, semiMajorAxis=40e4
        )
        exp = "Ellipse(-110.0, 40.0, 0.0)"
        self.assertEqual(repr(e), exp)

        e = cesiumpy.Cylinder(
            position=[-110, 40, 100], length=100e4, topRadius=10e4, bottomRadius=10e4
        )
    ...
    def test_outlineColor_property(self):
        ...
        with pytest.raises_regexp(traitlets.TraitError, msg):
            cesiumpy.Box(
                position=[-120, 40, 0], dimensions=(10, 20, 30), outlineColor=1
            )

        b = cesiumpy.Box(
            position=[-120, 40, 0], dimensions=(10, 20, 30), outlineColor="red"
        )
