...
class TestGeocode:
    def test_geocode(self):
        try:
            ...
            self.assertEqual(
                result, [(-118.2436849, 34.0522342), (-115.1398296, 36.1699412)]
            )

            result = cesiumpy.geocode._maybe_geocode(
                ["Los Angeles", "Las Vegas", [1, 2]]
            )
            self.assertEqual(
                result, [(-118.2436849, 34.0522342), (-115.1398296, 36.1699412), [1, 2]]
            )
            ...
            result = cesiumpy.geocode._maybe_geocode(
                "xxxxx_str_unable_to_be_converted_xxxxxx"
            )
    ...
    def test_geocode_unicode(self):
        try:
            result = cesiumpy.geocode._maybe_geocode("富士山")
            self.assertEqual(result, (138.7277777, 35.3605555))

            result = cesiumpy.geocode._maybe_geocode("xxxxx_変換できない文字列_xxxxxx")
            self.assertEqual(result, "xxxxx_変換できない文字列_xxxxxx")

            result = cesiumpy.geocode._maybe_geocode(["富士山", "赤岳"])
            self.assertEqual(
                result, [(138.7277777, 35.3605555), (138.3702338, 35.9709019)]
            )
        except geopy.exc.GeocoderQuotaExceeded:
            raise nose.SkipTest("exceeded geocoder quota")
    ...
    def test_cartesian3_geocode(self):
        try:
            result = cesiumpy.entities.cartesian.Cartesian3.maybe("富士山")
            self.assertEqual(
                result.script, "new Cesium.Cartesian3(138.7277777, 35.3605555, 0.0)"
            )

            result = cesiumpy.entities.cartesian.Cartesian3.maybe("Los Angeles")
            self.assertEqual(
                result.script, "new Cesium.Cartesian3(-118.2436849, 34.0522342, 0.0)"
            )
        except geopy.exc.GeocoderQuotaExceeded:
            raise nose.SkipTest("exceeded geocoder quota")
    ...
    def test_entities_geocode(self):
        try:
            ...
            e = cesiumpy.Ellipse(
                position="Los Angeles", semiMinorAxis=25e4, semiMajorAxis=40e4
            )
            ...
            e = cesiumpy.Cylinder(
                position="Los Angeles", length=100e4, topRadius=10e4, bottomRadius=10e4
            )
            ...
            e = cesiumpy.Polygon(
                hierarchy=["Los Angeles", "Las Vegas", "San Francisco"]
            )
            ...
            e = cesiumpy.Wall(
                positions=["Los Angeles", "Las Vegas", "San Francisco"],
                maximumHeights=10e4,
                minimumHeights=0,
            )
            ...
            e = cesiumpy.Corridor(
                positions=["Los Angeles", "Las Vegas", "San Francisco"], width=2e5
            )
            ...
            e = cesiumpy.Polyline(
                positions=["Los Angeles", "Las Vegas", "San Francisco"], width=0.5
            )
