...
@pytest.mark.skip("Geocode key required")
class TestGeocode:
    def test_geocode(self):
        try:
            result = cesiumpy.geocode._maybe_geocode("Los Angeles")
            assert result == (-118.2436849, 34.0522342)

            result = cesiumpy.geocode._maybe_geocode(["Los Angeles", "Las Vegas"])
            assert result == [(-118.2436849, 34.0522342), (-115.1398296, 36.1699412)]
    ...
    def test_geocode_unicode(self):
        try:
            ...
            assert result == (138.7277777, 35.3605555)

            result = cesiumpy.geocode._maybe_geocode("xxxxx_変換できない文字列_xxxxxx")
            assert result == "xxxxx_変換できない文字列_xxxxxx"

            result = cesiumpy.geocode._maybe_geocode(["富士山", "赤岳"])
            assert result == [(138.7277777, 35.3605555), (138.3702338, 35.9709019)]
        except geopy.exc.GeocoderQuotaExceeded:
            return
