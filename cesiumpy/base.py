...
class _CesiumBase(_CesiumObject):
    ...
    def __init__(
        ...
        self.zoom_to_entity: bool = (
            zoom_to_entity if (zoom_to_entity is not None) else DEFAULT_ZOOM_TO_ENTITY
        )
        self.track_entity: bool = (
            track_entity if (track_entity is not None) else DEFAULT_TRACK_ENTITY
        )
    ...
    def _setup_scripts(self) -> List[str]:
        ...
        if self._default_access_token:
            setup_scripts.append(
                f'Cesium.Ion.defaultAccessToken = "{self._default_access_token}";'
            )
    ...
    def _widget_scripts(self) -> List[str]:
        ...
        if props != "":
            ...
            script = script.format(
                varname=self._varname, klass=self._klass, id=self.div.id
            )
        return [script]
